...
solve vec (n, w) = U.maximum $ U.findIndices (\cw -> cw /= infWeight && cw <= w) $ dyna phi psi (n-1)
  where
    
    infWeight = 10^9+1
    limVal = U.foldr (\(_, v) t -> t+v) 0 vec
    
    psi 0 = NonEmptyListF (vec U.! 0) Nothing
    psi i = NonEmptyListF (vec U.! i) (Just (i-1))

    phi :: NonEmptyListF (Cofree NonEmptyListF (U.Vector Weight)) -> U.Vector Weight
    phi (NonEmptyListF (cw, cv) Nothing) = U.generate (limVal+1) p
      where p i | i == 0 = 0
                | i == cv = cw
                | otherwise = infWeight
    phi (NonEmptyListF (cw, cv) (Just t)) = U.zipWith min prev vec
      where
        prev = extract t
        vec = U.replicate cv infWeight U.++ U.map p prev
          where
            p ow | ow == infWeight = infWeight
                 | otherwise = ow+cw
