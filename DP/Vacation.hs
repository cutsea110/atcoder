...
solve vec = max3 . dyna phi psi
  where
    psi 0 = NonEmptyListF (vec U.! 0) Nothing
    psi i = NonEmptyListF (vec U.! i) (Just (i-1))

    phi :: NonEmptyListF (Cofree NonEmptyListF Score) -> Score
    phi (NonEmptyListF abc Nothing) = abc
    phi (NonEmptyListF (a, b, c) (Just t)) =
      let (a', b', c') = extract t
      in (max (a+b') (a+c'), max (b+a') (b+c'), max (c+a') (c+b'))
