...
discount :: SkewHeap Int -> SkewHeap Int
discount hp = let Just (x, hp') = extractMax hp
              in hp' +++ node (x `div` 2)

discount' :: Int -> SkewHeap Int -> SkewHeap Int
discount' n hp | n == 0 = hp
               | otherwise = discount' (n-1) (discount hp)

main :: IO ()
main = do
  (n, m) <- getIntTuple :: IO (Int, Int)
  as <- fromList <$> getInts
  let ttl = sumSkewHeap $ discount' m as
  print ttl
